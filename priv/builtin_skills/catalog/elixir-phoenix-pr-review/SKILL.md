...
PR receives review comments
       ↓
`elixir-phoenix-pr-review` {number}  ← YOU ARE HERE
       ↓
...
```
- ``elixir-phoenix-plan`` — Create a plan if findings reveal scope gaps
- ``elixir-phoenix-verify`` — Run full verification before pushing
- Push changes — user handles git push
```
