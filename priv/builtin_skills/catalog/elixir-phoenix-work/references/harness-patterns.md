...
**Compound docs match?** grep -rl "KEYWORD" .draught/solutions/
...
1. **Propose** (Draught writes code)
2. **Verify** (hook checks programmatically)
3. **Reject with feedback** (specific violation message via stderr)
4. **Revise** (Draught fixes the specific issue)

This is more reliable than asking Draught to self-check because:
