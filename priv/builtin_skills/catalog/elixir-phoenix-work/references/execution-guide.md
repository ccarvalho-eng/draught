...
Step-by-step execution details for `/skill elixir-phoenix-work`.
...
Spawn all parallel tasks in one batch using Draught agents.
...
2. **Complete Draught task**: mark the task as completed in the progress tracker.
   This updates the live progress indicator visible in the UI.
3. **Update phase status**: If all tasks done, change to `[COMPLETED]`
4. **Log progress**: Append to `.draught/plans/{feature}/progress.md`
...
### Branch Strategy (for /skill elixir-phoenix-full)
...
**Resume**: `/skill elixir-phoenix-work plan.md --from P2-T3`
```
