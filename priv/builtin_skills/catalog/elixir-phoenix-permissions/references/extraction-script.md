...
allowed = []
for path in [os.path.expanduser('~/.draught/settings.json'),
             '.draught/settings.json', '.draught/settings.local.json']:
    try:
        with open(path) as f:
...
all_files = glob.glob(os.path.expanduser('~/.draught/projects/*/*.jsonl'))
