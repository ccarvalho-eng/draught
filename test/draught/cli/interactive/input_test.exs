defmodule Draught.CLI.Interactive.InputTest do
  ...
  test "parses commands without forwarding them as prompts" do
    ...
    assert Input.parse("/custom-skills") == {:ok, {:command, :"custom-skills", nil}}
    assert Input.parse("/builtin-skills") == {:ok, {:command, :"builtin-skills", nil}}

    assert Input.parse("/skill safe-review") ==
             {:ok, {:command, :skill, "safe-review"}}

    assert Input.parse("/skill safe-review lib/draught.ex") ==
             {:ok, {:command, :skill, "safe-review lib/draught.ex"}}
