...
Detailed templates for each section of the `/skill elixir-phoenix-brief` interactive
...
`.draught/plans/{slug}/scratchpad.md` DECISION entries
...
fix or architectural decision, suggest `/skill elixir-phoenix-compound` to capture it.}
```
...
- Suggest `/skill elixir-phoenix-compound` only when there's genuinely novel
...
3. If the answer isn't in the artifacts, say so: "The plan doesn't
   ...
   it or run `/skill elixir-phoenix-plan --existing` to research this aspect"
