...
1. **Read progress file** `.draught/plans/{slug}/progress.md`
2. **Check plan checkboxes** in `.draught/plans/{slug}/plan.md`:
...
```bash
/skill elixir-phoenix-full --resume magic-link-auth
# Reads .draught/plans/magic-link-auth/plan.md
...
```bash
/skill elixir-phoenix-work .draught/plans/magic-link-auth/plan.md --from P2-T3
```
...
```bash
/ralph-loop:ralph-loop "/skill elixir-phoenix-full {feature}" --completion-promise "DONE" --max-iterations 50
```
...
```
/skill elixir-phoenix-full {feature} --checkpoint-after plan
/skill elixir-phoenix-full {feature} --checkpoint-after each-phase
```
...
COMPLETED=$(grep -c '\[x\]' .draught/plans/${SLUG}/plan.md)
TOTAL=$(grep -c '\[.*\] \[P' .draught/plans/${SLUG}/plan.md)
