...
The interactive prompt separates input from responses with a rounded, open-sided frame and a `›` marker. Its top rail keeps the active model and workspace visible before the `/help` hint; paths inside the current user's home use `~`. The frame follows the detected terminal width and falls back to `>` on narrow terminals. While the session is idle, the prompt supports grapheme-aware cursor movement, Home, End, Backspace, Delete, logical-line Up and Down movement, Tab completion, and bracketed multiline paste. Enter submits. Shift+Enter inserts a newline when the terminal reports modified keys; Ctrl+J and Alt+Enter are portable fallbacks. Assistant segments use spacing without speaker labels; indented `Tool:` records identify tool activity. These distinctions remain visible with `--color never`. Redirected text and JSONL retain their plain output formats.
...
Anonymous, named-create, and resumed tasks use the same preparation boundary. A resumed invocation supplies its own policy; adapter configuration and earlier grants are not restored from the journal or session binding. Waiting for a human approval does not consume the active tool or interactive-turn execution budget; bounded work before and after the answer remains charged. This injection boundary does not add terminal prompts or a configuration-file setting.
...
| `/permissions` | Displays the active workspace, admitted risk classes, approval behavior, and web capability state. |
...
| `/rename NAME` | Changes the display name of the current idle session without changing its ID. A fresh name is persisted with the first turn. |
...
Model discovery preserves Ollama's inventory order and exposes only models that meet the complete agent capability requirement. Exact names take precedence over one-based positions, including when a model name is numeric. A model may change only while the current session is idle and has no durable history. `/model` saves that choice as the user default and applies it to fresh sessions opened by `/new`. Once a session establishes durable state, its journal binding fixes the provider and model; use `/new` before selecting another model. Resuming an existing session uses its recorded model instead of the user default. Command-line flags, environment variables, and project configuration remain explicit overrides and must match a resumed binding.

Session names are display metadata and need not be unique. Use a row number or immutable ID when a name is ambiguous. Each row shows one identity: an explicit name, otherwise a sanitized and trimmed preview of the latest successful user message, otherwise the immutable ID. Catalog views assign one-based positions, so `/resume 2` selects the second active record from the deterministic filtered list. Exact IDs and unique names take precedence over positions. Session catalog discovery is read-only and bounded; it does not replay journals. Exact IDs use direct lookup, so known sessions can still be archived or restored when a complete listing exceeds its entry limit. Corrupt or unsafe records are shown only by their validated ID as unavailable and cannot be selected. Archived sessions are rejected at the storage boundary for both interactive and headless resume until restored.
