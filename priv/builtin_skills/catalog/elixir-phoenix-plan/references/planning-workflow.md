...
Full step-by-step details for `/skill elixir-phoenix-plan`. The SKILL.md has a
summary; this reference has the complete workflow.

## Interview Detection (from /skill elixir-phoenix-brainstorm)
...
2. Check `.draught/plans/*/interview.md` for recent files (<24h)
...
- Note in scratchpad: "Requirements from /skill elixir-phoenix-brainstorm interview"
...
`.draught/plans/{slug}/scratchpad.md` as DECISION entries for future
reference.
...
Then read reports from `.draught/plans/{slug}/research/`.
...
to `.draught/plans/{slug}/scratchpad.md` under a `## Infrastructure`
...
Create plan(s) at `.draught/plans/{feature-slug}/plan.md`.

Key requirements:

- Tasks in `- [ ] [Pn-Tm][annotation] Description` format
  (required for `/skill elixir-phoenix-work`). Valid annotations:
...
**Scratchpad**: Also create `.draught/plans/{feature-slug}/scratchpad.md`
...
count, phase names, key scope). Then use `ask the user in chat`:

For single plan:

- **Start in fresh session** (recommended for 5+ tasks)
- **Get a briefing** -- interactive walkthrough via `/skill elixir-phoenix-brief`
...
Do NOT say "Start Phase 1" — `/skill elixir-phoenix-work` runs the whole plan.

**When user selects "Start in fresh session"**, print clear
step-by-step:

```
1. Start a fresh Draught session
2. Then run one of:
   /skill elixir-phoenix-work .draught/plans/{slug}/plan.md
   /skill elixir-phoenix-full .draught/plans/{slug}/plan.md  (includes review + compound)
```
...
2. **Search compound docs** -- Find known issues in planned areas
   (`grep -rl "KEYWORD" .draught/solutions/`)
3. **Spawn research agents** -- Use SPECIALIST agents (same
   ...
   `.draught/plans/{slug}/research/{topic}.md` and return ONLY a
