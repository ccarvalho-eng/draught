...
Detailed methodology for the adaptive interview phase of `/skill elixir-phoenix-brainstorm`.
...
Write to `.draught/plans/{slug}/interview.md`:
...
- **Summary section** is what `/skill elixir-phoenix-plan` reads first — must be self-contained
- **Coverage Details** replace `/skill elixir-phoenix-plan`'s clarification questions
- **Codebase Context** replaces the patterns-analyst research in `/skill elixir-phoenix-plan`
- **Transcript** at the bottom for audit trail — not consumed by `/skill elixir-phoenix-plan`
