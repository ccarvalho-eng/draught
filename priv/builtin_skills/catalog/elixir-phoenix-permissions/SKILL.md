...
metadata:
  short-description: 'Recommend Draught command permissions'
---
...
Walk through findings interactively using `ask the user in chat`. Present items
in batches by risk level, starting with GREEN (safest):

**Batch 1 — GREEN items** (read-only, tests, safe tools):
Use `ask the user in chat` with options:
...
Always show individually — one `ask the user in chat` per item with options:
...
Merge approved additions into `~/.draught/settings.json` under `permissions.allow`.
