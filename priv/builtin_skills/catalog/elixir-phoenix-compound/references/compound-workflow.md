...
```
/skill elixir-phoenix-compound Fixed N+1 query in UserListLive — was missing preload on posts
```
...
1. **Scratchpad DEAD-END and DECISION entries**:
   ...
   ```bash
   grep -A 5 "DEAD-END\|DECISION" .draught/plans/*/scratchpad.md 2>/dev/null | tail -20
   ```
...
3. **Progress file completions**:
   ...
   ```bash
   ls -t .draught/plans/*/progress.md 2>/dev/null | head -1
   ```
...
```bash
grep -rl "NotLoaded\|timeout\|N+1" .draught/solutions/ 2>/dev/null
grep -rl "module: \"Accounts\"" .draught/solutions/ 2>/dev/null
```
...
Create in `.draught/solutions/{category}/` using the template from
...
- `/skill elixir-phoenix-learn-from-fix` captures quick patterns in `common-mistakes.md`
- `/skill elixir-phoenix-compound` captures detailed solutions with full context
- `/skill elixir-phoenix-investigate` searches `.draught/solutions/` before investigating
- `/skill elixir-phoenix-plan` consults for known risks in planned areas
