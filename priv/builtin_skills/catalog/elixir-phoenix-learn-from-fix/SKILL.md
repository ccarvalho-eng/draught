...
- Search project AGENTS.md for the pattern keyword
...
`~/.draught/plugins/` are cached and get overwritten on updates.
...
| Detailed fix with context | `.draught/solutions/` via ``elixir-phoenix-compound`` | Complex debugging story |
...
`~/.draught/projects/{project-hash}/memory/MEMORY.md`:
...
- Suggest adding to project AGENTS.md Iron Laws section
...
1. **NEVER edit plugin files** — files under `~/.draught/plugins/` are cached and overwritten on updates
2. **DO NOT duplicate existing lessons** — always check AGENTS.md and memory before writing
