...
- Modify any files outside `.draught/plans/{slug}/reviews/`
- Start fixing issues

Task creation and planning happen in `/skill elixir-phoenix-plan` after the user decides.
...
Ready for: /skill elixir-phoenix-learn-from-fix (capture lessons)
```
...
```
Review found {n} untested public functions/callbacks:

1. {Function/callback} in {file} -- no test coverage
2. {Function/callback} in {file} -- no test coverage

How would you like to proceed?

- `/skill elixir-phoenix-plan` — Plan tests for these functions
- `/skill elixir-phoenix-work` — Write tests directly
...
- `/skill elixir-phoenix-plan .draught/plans/{slug}/reviews/{file}.md` — Replan fixes
  (best for complex or architectural issues)
- `/skill elixir-phoenix-work` — Fix directly
