...
`elixir-phoenix-brief` .draught/plans/user-auth/plan.md    # Specific plan
```
...
2. Otherwise, find latest plan:
   ...
   Use `rg --files` to find `.draught/plans/*/plan.md` and pick the most recent.
...
- `.draught/plans/{slug}/summaries/consolidated.md` (research summary)
- `.draught/plans/{slug}/scratchpad.md` (decisions, dead-ends)
- `.draught/plans/{slug}/progress.md` (work log, post-work only)
...
section, use `ask the user in chat` with options:
