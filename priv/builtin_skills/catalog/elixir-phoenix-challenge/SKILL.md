...
### Ecto Challenge (``elixir-phoenix-challenge` ecto`)
...
### LiveView Challenge (``elixir-phoenix-challenge` liveview`)
...
### PR Challenge (``elixir-phoenix-challenge` pr`)
...
1. **Search** `.draught/plans/*/reviews/` and `.draught/reviews/` for prior findings
...
Run ``elixir-phoenix-challenge` [mode]` to initiate a rigorous review. The reviewer will not approve until all concerns are addressed with evidence.

Example workflow:

1. Run ``elixir-phoenix-challenge` ecto` after migration changes
