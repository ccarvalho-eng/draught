...
| Type | Path |
|------|------|
| Plan | .draught/plans/{slug}/plan.md |
| Progress | .draught/plans/{slug}/progress.md |
| Review | .draught/plans/{slug}/reviews/{slug}-review.md |
...
**Phase**: 2
**Task**: 3/4
**Description**: Implement verify_magic_token/1
**Attempt**: 2/3

## Session Log

### 11:45:00 - Task Failed
- Task: Implement verify_magic_token/1
- Error: Test assertion failed
- Retry: 1/3

### 11:42:00 - Task Completed
- Task: Implement create_magic_token/1
- Files: lib/my_app/auth.ex
- Verification: PASS

### 11:35:00 - Task Completed
- Task: Generate Auth context
- Files: lib/my_app/auth.ex, lib/my_app/auth/magic_token.ex
- Verification: PASS
```
...
When context window fills, Draught loses memory. Recovery:
...
- Progress file: .draught/plans/{slug}/progress.md
- Current state: WORKING
- Current phase: 2
- Current task: 3/4
- Last error: {error from progress file}

Continue with /skill elixir-phoenix-work .draught/plans/{slug}/plan.md
```
...
ls -t .draught/plans/*/progress.md | head -1

# Check state
grep "State:" .draught/plans/*/progress.md | head -1
...
# Then resume
/skill elixir-phoenix-work .draught/plans/{slug}/plan.md --from {task-id}
```
...
```
Phase 2: Context     ████████████
Phase 3: LiveView         ████████████  (starts when context API stable)
...
```bash
# Create PR
gh pr create --title "feat: {feature}" --body "$(cat .draught/plans/{slug}/reviews/{slug}-review.md)"
