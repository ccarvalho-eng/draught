...
`elixir-phoenix-plan` .draught/plans/notifications/reviews/notifications-review.md
`elixir-phoenix-plan` Implement notifications --depth deep
`elixir-phoenix-plan` .draught/plans/auth/plan.md --existing
```
...
5. **Spawn research agents** — Selective, parallel, based on need.
   Create a Draught task per agent for progress visibility:
...
1. Load plan, search `.draught/solutions/` for known risks
2. Spawn SPECIALIST agents (not Explore) for thin sections.
   Each agent writes to `.draught/plans/{slug}/research/` and
...
`elixir-phoenix-plan` {feature}  <-- YOU ARE HERE
   ...
   `elixir-phoenix-plan` --existing (optional enhancement)
       |
   ASK USER -> `elixir-phoenix-work` .draught/plans/{feature}/plan.md
       |
...
- Plans saved to `.draught/plans/{slug}/plan.md`
- Research reports in `.draught/plans/{slug}/research/` can be deleted after

## CRITICAL: After Writing the Plan

**STOP. Do NOT proceed to implementation.**

After writing `.draught/plans/{slug}/plan.md`:

1. Summarize: task count, phases, key decisions
2. Use `ask the user in chat` with options:
...
2. Then run one of:
   `elixir-phoenix-work` .draught/plans/{slug}/plan.md
   `elixir-phoenix-full` .draught/plans/{slug}/plan.md  (includes review + compound)
```
